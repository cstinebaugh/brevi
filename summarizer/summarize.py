...
def extractSentences(text, length):
    global senToken
    sentence = clean_text_by_sentences(text, language="english")
...
try:
    opts, args = getopt.getopt(sys.argv[1:], 'f:l:', ['file=', 'length='])
...
with open(file_name, 'r') as file:
    origText = file.read().replace('\n', ' ')
print(extractSentences(origText, int(length)))